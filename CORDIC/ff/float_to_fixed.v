...
//altfp_convert CBX_AUTO_BLACKBOX="ALL" DEVICE_FAMILY="Cyclone V" OPERATION="FLOAT2FIXED" ROUNDING="TO_NEAREST" WIDTH_DATA=32 WIDTH_EXP_INPUT=8 WIDTH_EXP_OUTPUT=8 WIDTH_INT=2 WIDTH_MAN_INPUT=23 WIDTH_MAN_OUTPUT=23 WIDTH_RESULT=18 aclr clk_en clock dataa result
...
//altbarrel_shift CBX_AUTO_BLACKBOX="ALL" DEVICE_FAMILY="Cyclone V" PIPELINE=2 SHIFTDIR="VARIABLE" SHIFTTYPE="LOGICAL" WIDTH=40 WIDTHDIST=6 aclr clk_en clock data direction distance result
...
module  float_to_fixed_altbarrel_shift_b4h
	( 
	aclr,
	clk_en,
	clock,
...
`endif
	...
	reg	[1:0]	dir_pipe;
	reg	[39:0]	sbit_piper1d;
	reg	[39:0]	sbit_piper2d;
	reg	sel_pipec3r1d;
	reg	sel_pipec4r1d;
	reg	sel_pipec5r1d;
	wire  [6:0]  dir_w;
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) sbit_piper1d <= 40'b0;
		else if  (clk_en == 1'b1)   sbit_piper1d <= smux_w[119:80];
	// synopsys translate_off
	initial
		sbit_piper2d = 0;
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) sbit_piper2d <= 40'b0;
		else if  (clk_en == 1'b1)   sbit_piper2d <= smux_w[239:200];
	// synopsys translate_off
	initial
		sel_pipec3r1d = 0;
	// synopsys translate_on
	...
	assign
		...
		pad_w = {32{1'b0}},
		result = sbit_w[279:240],
		sbit_w = {sbit_piper2d, smux_w[199:120], sbit_piper1d, smux_w[79:0], data},
		sel_w = {sel_pipec5r1d, sel_pipec4r1d, sel_pipec3r1d, distance[2:0]},
		smux_w = {((({40{(sel_w[5] & (~ dir_w[5]))}} & {sbit_w[207:200], pad_w[31:0]}) | ({40{(sel_w[5] & dir_w[5])}} & {pad_w[31:0], sbit_w[239:232]})) | ({40{(~ sel_w[5])}} & sbit_w[239:200])), ((({40{(sel_w[4] & (~ dir_w[4]))}} & {sbit_w[183:160], pad_w[15:0]}) | ({40{(sel_w[4] & dir_w[4])}} & {pad_w[15:0], sbit_w[199:176]})) | ({40{(~ sel_w[4])}} & sbit_w[199:160])), ((({40{(sel_w[3] & (~ dir_w[3]))}} & {sbit_w[151:120], pad_w[7:0]}) | ({40{(sel_w[3] & dir_w[3])}} & {pad_w[7:0], sbit_w[159:128]})) | ({40{(~ sel_w[3])}} & sbit_w[159:120])), ((({40{(sel_w[2] & (~ dir_w[2]))}} & {sbit_w[115:80], pad_w[3:0]}) | ({40{(sel_w[2] & dir_w[2])}} & {pad_w[3:0], sbit_w[119:84]})) | ({40{(~ sel_w[2])}} & sbit_w[119:80])), ((({40{(sel_w[1] & (~ dir_w[1]))}} & {sbit_w[77:40], pad_w[1:0]}) | ({40{(sel_w[1] & dir_w[1])}} & {pad_w[1:0], sbit_w[79:42]})) | ({40{(~ sel_w[1])}} & sbit_w[79:40])), ((({40{(sel_w[0] & (~ dir_w[0]))}} & {sbit_w[38:0], pad_w[0]}) | ({40{(sel_w[0] & dir_w[0])}} & {pad_w[0], sbit_w[39:1]})) | ({40{(~ sel_w[0])}} & sbit_w[39:0]))};
endmodule //float_to_fixed_altbarrel_shift_b4h

//synthesis_resources = lpm_add_sub 6 lpm_compare 4 reg 226 
//synopsys translate_off
`timescale 1 ps / 1 ps
//synopsys translate_on
module  float_to_fixed_altfp_convert_b1o
	( 
	aclr,
	clk_en,
	clock,
...
`endif
	...
	wire  [39:0]   wire_altbarrel_shift2_result;
	...
	reg	int_or_reg2;
	reg	int_or_reg3;
	reg	[17:0]	integer_result_reg;
	reg	[16:0]	integer_rounded_reg;
	reg	man_or1_reg1;
	reg	man_or2_reg1;
	reg	man_or_reg2;
	reg	man_or_reg3;
	...
	wire  [16:0]   wire_add_1_adder_result;
	...
	wire  add_1_cout_w;
	wire  add_1_w;
	wire  [17:0]  all_zeroes_w;
	wire  barrel_direction_negative;
	wire  [39:0]  barrel_mantissa_input;
	wire  [15:0]  barrel_zero_padding_w;
	wire  below_limit_exceeders;
	wire  [17:0]  below_limit_integer;
	...
	wire  [17:0]  exceed_limit_integer;
	...
	wire  [17:0]  infinity_value_w;
	wire  int_or1_w;
	wire  int_or2_w;
	wire  [17:0]  integer_output;
	wire  [16:0]  integer_post_round;
	wire  [16:0]  integer_pre_round;
	wire  [17:0]  integer_result;
	wire  [16:0]  integer_rounded;
	wire  [16:0]  integer_rounded_tmp;
	wire  [16:0]  integer_tmp_output;
	wire  [7:0]  inv_add_1_adder1_w;
	wire  [8:0]  inv_add_1_adder2_w;
	wire  [16:0]  inv_integer;
	wire  [39:0]  lbarrel_shift_result_w;
	wire  [16:0]  lbarrel_shift_w;
	wire  lowest_integer_selector;
	wire  [16:0]  lowest_integer_value;
	wire  [10:0]  man_bus1;
	wire  [11:0]  man_bus2;
	wire  [10:0]  man_or1;
	wire  man_or1_w;
	...
	wire  nan_input_w;
	wire  [17:0]  neg_infi_w;
	wire  [7:0]  padded_exponent_input;
	wire  [17:0]  pos_infi_w;
	...
	wire  sign_input;
	wire  sign_input_w;
	wire  [16:0]  signed_integer;
	wire  sticky_bit_w;
	wire  [21:0]  sticky_bus;
	wire  [21:0]  sticky_or;
	wire  [16:0]  unsigned_integer;
	wire  upper_limit_w;
	wire  zero_input_w;

	float_to_fixed_altbarrel_shift_b4h   altbarrel_shift2
	( 
	.aclr(aclr),
	.clk_en(clk_en),
	.clock(clock),
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) integer_result_reg <= 18'b0;
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) integer_rounded_reg <= 17'b0;
	...
	assign
		...
		barrel_zero_padding_w = {16{1'b0}},
		below_limit_exceeders = ((((denormal_input_w | zero_input_w) | nan_input_w) | below_lower_limit3_reg4) & (~ border_lower_limit_reg4)),
		below_limit_integer = (({18{(~ below_limit_exceeders)}} & integer_output) | ({18{below_limit_exceeders}} & all_zeroes_w)),
		...
		exceed_limit_integer = (({18{(~ exceed_limit_exceeders)}} & below_limit_integer) | ({18{exceed_limit_exceeders}} & infinity_value_w)),
		...
		infinity_value_w = (({18{(~ sign_input_w)}} & pos_infi_w) | ({18{sign_input_w}} & neg_infi_w)),
		int_or1_w = man_or2[0],
		int_or2_w = man_or1[6],
		...
		integer_rounded = (({17{(~ lowest_integer_selector)}} & integer_rounded_tmp) | ({17{lowest_integer_selector}} & lowest_integer_value)),
		integer_rounded_tmp = (({17{(~ add_1_w)}} & integer_pre_round) | ({17{add_1_w}} & integer_post_round)),
		integer_tmp_output = (({17{(~ sign_input_w)}} & unsigned_integer) | ({17{sign_input_w}} & signed_integer)),
		inv_add_1_adder1_w = wire_add_sub3_result,
		inv_add_1_adder2_w = (({9{(~ wire_add_sub3_cout)}} & inv_integer[16:8]) | ({9{wire_add_sub3_cout}} & wire_add_sub4_result)),
		inv_integer = (~ integer_rounded_reg),
		lbarrel_shift_result_w = wire_altbarrel_shift2_result,
		lbarrel_shift_w = lbarrel_shift_result_w[39:23],
		...
		neg_infi_w = {1'b1, {17{1'b0}}},
		padded_exponent_input = exponent_input,
		pos_infi_w = {1'b0, {17{1'b1}}},
...
endmodule //float_to_fixed_altfp_convert_b1o
//VALID FILE
...
module float_to_fixed (
	...
	float_to_fixed_altfp_convert_b1o	float_to_fixed_altfp_convert_b1o_component (
				.aclr (aclr),
				.clk_en (clk_en),
				.clock (clock),
				.dataa (dataa),
